...
var linters = {
  ...
  possibleInvalidLinkTags: function possibleInvalidLinkTags(analyzer) {
    var invalidTags = [];
    var isPossibleInvalidLinkTag = p.AND(
      p.hasTagName('link'),
      p.hasAttr('href'),
      p.hasAttr('rel')
    );
    var nodes = analyzer.nodeWalkAllDocuments(isPossibleInvalidLinkTag);

    nodes.forEach(function(node) {
      var href = dom5.getAttribute(node, 'href');
      var rel = dom5.getAttribute(node, 'rel');

      if(/\.html$/.test(href) && rel !== 'import') {
        invalidTags.push(lintErrorFromNode(
          node,
          "HTML being imported as stylesheet, did you mean 'rel=\"import\"'?",
          false
        ));
      }
    });
